<template>
  <section class="container  q-mx-auto">
    ...
    >
      <q-carousel-slide v-for="item in contentList" :name="item.id" class=" grid lg:grid-cols-2 md:grid-cols-1 items-center gap-y-3 overflow-hidden justify-between leading-tight  ">
        ...
        <div grid >
          ...
          <q-btn @click="path(item)" class="normal-case text-20px rounded-lg c-#39B44A w-30% justify-self-center transition-duration-400" md="justify-self-end" hover="bg-white shadow-sm shadow-#39B44A font-600 transition-duration-400" unelevated>{{lang.getLang('more')}}</q-btn>
        </div>
      </q-carousel-slide>
    </q-carousel>
  </section>
